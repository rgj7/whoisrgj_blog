...
function MetaField({ label, children }) {
  return (
    <div>
      <dt className="text-xs font-semibold text-stone-500 dark:text-navy-400 uppercase tracking-widest mb-0.5">
        {label}
      </dt>
      <dd className="text-sm text-stone-800 dark:text-navy-100">{children}</dd>
    </div>
  )
}
...
export default function GameInfoPanel({ gameId }) {
  ...
  return (
    <div className="border-y border-stone-300/50 dark:border-navy-700/50 bg-stone-100 dark:bg-navy-950 overflow-hidden">
      <div className="flex flex-col md:flex-row py-7 px-8">
        {/* Left — description */}
        {hasDescription && (
          <div className="flex-1 pb-4 md:pb-0 md:pr-5 border-b border-stone-300/50 dark:border-navy-700/50 md:border-b-0 md:border-r">
            <h2 className="text-xs font-semibold text-stone-500 dark:text-navy-400 uppercase tracking-widest mb-2">
              About the Game
            </h2>
            <p
              className={`text-sm text-stone-800 dark:text-navy-100 leading-relaxed whitespace-pre-line ${!expanded ? 'line-clamp-4' : ''}`}
            >
            ...
            {game.description_raw.length > 280 && (
              <button
                ...
                className="mt-1.5 text-xs text-stone-500 dark:text-navy-400 hover:text-stone-600 dark:hover:text-navy-200 transition-colors"
              >
        ...
        <div className="shrink-0 md:w-40 pt-4 md:pt-0 md:pl-5 flex flex-col gap-2.5">
          ...
          {game.esrb_rating && (
            <MetaField label="ESRB">
              <span className="font-mono text-xs bg-white dark:bg-navy-800 border border-stone-200 dark:border-navy-600 text-stone-800 dark:text-navy-100 px-2 py-0.5 rounded">
                {game.esrb_rating}
              </span>
            </MetaField>
          )}
      ...
      <div className="border-t border-stone-300/50 dark:border-navy-700/50 px-8 py-4 flex flex-wrap items-start gap-x-8 gap-y-3">
        ...
        <div className="ml-auto self-end">
          <a
            ...
            className="text-xs text-stone-500 dark:text-navy-400 hover:text-stone-600 dark:hover:text-navy-200 transition-colors"
          >
            Data via RAWG ↗
          </a>
        </div>
